...
import { BehaviorSubject , concatMap , filter , finalize , forkJoin , from , map , Observable , switchMap , toArray , } from 'rxjs';
...
import { Layout, Col } from 'src/app/core/models/user-settinng.interface'
...
export class ValidazioneComponent implements OnInit {
  parameters$: BehaviorSubject<Array<IParameter>> = new BehaviorSubject<Array<IParameter>>([]);
  ...
  openDialogPrameters() {
    ...
    dialogRef.afterClosed().subscribe( ( data ) => {
      ...
      selected.parametri.forEach( ( element ) => {
        all
          .filter( ( {name} ) => name == element.name )
          .forEach( ( elementParams ) => {
            this.parameters.push( this.dataService.createParameter( count , selected , elementParams ) );
          } );
      } );
